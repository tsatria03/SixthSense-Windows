...
- F2 goes to the next level, the way the end of a level does (`_level_transition`), boss or no boss. After level 8 it goes round to level 1, with level 1's zombies (`debug.MAX_LEVEL`). While a level is changing it says "Not while the level is changing", and Shift+F2 says "Not while the section is changing".
