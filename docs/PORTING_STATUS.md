...
| `GoldStoreController`, `CoinStoreController`, `getCoinListController`, `StoreController.ItemAllAction:`, `mainStoreController.restoreAction:` | StoreKit. Buying gold or coins, the *Purchase all weapons* bundle and restoring purchases were in-app purchases. The restore purchases row is left out; the others are kept and say they are unavailable, while it is decided whether to remove them or let them be bought with gold. |
