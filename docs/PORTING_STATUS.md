...
| **port addition** | `game/debug.py`, `--debug` | Debug mode: nothing takes a heart and nothing you kill counts; every weapon is on Tab and nothing runs out. F2 next level (round to 1 after 8), Shift+F2 next section, F5 and Shift+F5 spawn, F6 hold the zombies, F7 let them hit you for no heart, F11 say where they are; the F1 screen lists them only in debug mode. See `DIVERGENCES.md`. |
