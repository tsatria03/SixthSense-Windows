...
- [Screen reader mode](project_screen_reader_mode.md): planned 2026-09-22. With the voice over row off, the screen reader speaks every `speech/` recording's words and reads numbers whole, while `sfx/` keeps playing; new players start self-voiced. The design, the prerequisites (transcripts, a speech layer without comtypes) and the stages. Not built yet.
- [Prism speech](project_prism_speech.md): decided 2026-09-22, not built yet. `speech.py` keeps NVDA first through its DLL, then Prism for JAWS, Narrator and the rest (check `narrator.exe`), then SAPI through Prism instead of comtypes. `compiler.py` needs Prism's flags, and a `licenses` folder in the release carries Prism, pygame, OpenAL Soft and the NVDA client (the last two are in `vendor/`).
