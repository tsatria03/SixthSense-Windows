...
5. **A grenade scores every zombie it damages.** The grenade loop sends `MonsterKillCount:` per damaged monster (0x3a53c..0x3a546) before the HP check, so score and top score rise for survivors; kills and gold do not (`killMonsterCount++` at 0x3a636 is on the kill path). Port: score only on a kill (`stage_1_e.py:1000-1005`).
