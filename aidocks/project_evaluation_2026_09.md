...
- [V] **Pausing during the 2 s level change starts the walk under the panel.** **Fixed 2026-09-23, confirmed by the dev:** `_pause` cancels `ChangeLevel_`, `levelChanging` remembers it, `continueAction_` schedules it again for `LEVEL_CHANGE_SECONDS` instead of starting the walk, and `gameReplayAction_` cancels it; two tests in `tests/case/pause.py`. Was: `_level_transition` schedules `ChangeLevel_` (`stage_1_e.py` about 526), and `StopPlayAction_` does not cancel it. The original is the same: `ChangeLevel:` builds the timer unconditionally (0x323c2), and `StopPlayAction:` has no `cancelPreviousPerformRequests`. Restarting from the panel inside that window gives two walk timers, because `gameReplayAction_` starts one and the pending `ChangeLevel_` adds another. That makes the player walk at double speed.
- [V] **The coin row's minutes can talk over the next row.** **Fixed 2026-09-23:** `AppDelegate.readStop` now cancels `readTimeMin`/`readTimeSec` and stops the word sounds; `test_moving_off_the_coin_row_stops_the_time_to_the_next_coin`. `readNumber_` schedules `readTimeMin`/`readTimeSec` on the app (`app_delegate.py` about 237, 245). `MainController.StopElseSpeak` cancels only its own `readNumberOfCoin`, and `readStop` does not cancel these. Whether the original cancels them is not checked.
- [V] **The intro's earphone warning (234) is not stopped by moving rows.** **Fixed 2026-09-23, confirmed by the dev:** `StartIntroPage.StopElseSpeak` stops 234 and cancels `sound_earphone`, and it is scheduled only behind the opening welcome, never on a reread of row 1 (the dev heard that as a duplicate, since the welcome already says it); two tests in `tests/case/intro.py`. `BlindScreen.StopElseSpeak` stops 14 and 266 but not 234, and it does not cancel `sound_earphone`, so the warning can land on top of row 2 or on a restarted welcome (`intro.py` about 102).
...
## Binary recheck, 2026-09-24 (at `bf02459`, after the sound sort and the run loop fixes)

Read-only sweep by an agent, each item checked in the raw bytes; Claude re-checked the port side of 1 and 2. All nine are in `docks/todo list.txt`'s `##Unfinished.`, same order. The full suite passed 321 of 321 at the same commit.

1. **Fixed 2026-09-24:** `_pause` performs `spaekMenu` after `PAUSED_VOICE_DELAY` (0.5, the raw `movt r3, #0x3fe0` at 0x34720 checked by Claude), for the stage, the tutorial and the test range; two tests in `pause.py`. **Pausing never says "paused".** `StopPlayAction:` schedules `spaekMenu` 0.5 s after the click (0x34710..0x34732, `movt r3, #0x3fe0`); it plays 229 at 0.2. The tutorial (0x8413e) and the test range (0x482a2) do the same. Port: `_pause` (`stage_1_e.py:1658`) plays only 10; `spaekMenu` (1637) is never called, and PORTING_STATUS calls it ported. In screen reader mode it should speak "Paused".
2. **Fixed 2026-09-24:** `tutorial_beat_end` is silent (Claude read `tutorialSixEnd` and `...RestartFinger`, finger only, and checked the selrefs: every Restart but Six's and Seven's is sent); the test now checks no replay, and DIVERGENCES is corrected. **The reload lesson's prompt (280) repeats about every 10 s.** The `tutorialNEnd` methods play no sound (0x8cb60..0x8cc62, 0x8dd78..0x8de7c, inline 0x3d488, 0x3e6a0); only `tutorialNRestart` replays, and `tutorialSixRestart`/`tutorialSevenRestart` are never sent. Port: `NAGGED` includes Six (`stage_tutorial.py:101`) and `CheckTutorial`/`tutorial_beat_end` (260-283) replay it with its key hint. DIVERGENCES' "CheckTutorial only nags One to Six" rests on this. The comment "0x8cb4a: the prompt has finished" (236) is wrong: that is `movs r2, #1`, the monster type for `MonsterInit:`.
3. **Fixed 2026-09-24:** `WEAPON_CHANGE_GAIN` 0.2 at z 40, and the old weapon's change sound stopped first (raw bytes checked by Claude); test in `gameplay.py`. **The weapon change sound is 1.0, not 0.2.** `gunChangeAction:` hard-codes 0.2 at z 40 (0x35e80/0x35e90, sent 0x35e9e) and first stops the old weapon's change sound (0x35c72/0x35c88). Port: `stage_1_e.py:1252-1254` uses the plist gain (1.0 everywhere) at z 0. Batch 3 had noted it as not yet ported; the status check dropped it.
4. **Undone at the dev's request, 2026-09-24:** by ear the original's points left shots off from the zombies in their lane, so shots and empty clicks are back on `_lane_pos`, a recorded divergence. Before that, **fixed 2026-09-24:** `GUN_SHOT_POS` at z 40, each point checked in the raw bytes by Claude; the melee miss keeps `_lane_pos`, since its x could not be pinned down (it comes from a value saved before 0x39c36). DIVERGENCES corrected. **Gunshots are panned too hard.** The original pans each gun lane at z 40: lane 1 (-25, 0), 2 (-15, 25), 3 (0, 25), 4 (15, 25), 5 (25, 0) (0x2f948, 0x2fbc6, 0x2f4a0, 0x2f7f8, 0x2f680, 0x2fbbe), about 21 to 32 degrees off centre; the grenade plays from (0, 0). Port: `_lane_pos` (`stage_1_e.py:875-893`) puts lane 1 hard left. DIVERGENCES' "Shots and swings are heard down their lane" is half a misreading; a melee miss really is at (0, 0) (0x39c36).
5. **Fixed 2026-09-24:** the grenade loop sends `MonsterKillCount_` and bumps `gunEggCountShot` per monster, and `_monster_killed(tally=False)` keeps a grenade kill from being tallied twice. Claude decoded the stack slots in the raw bytes: `[sp, #0x28]` is `MonsterKillCount:`, `[sp, #0x20]`/`#0x24` `gunEggCountShot`/its setter. Test in `gameplay.py`. **A grenade scores every zombie it damages.** The grenade loop sends `MonsterKillCount:` per damaged monster (0x3a53c..0x3a546) before the HP check, so score and top score rise for survivors; kills and gold do not (`killMonsterCount++` at 0x3a636 is on the kill path). Port: score only on a kill (`stage_1_e.py:1000-1005`).
6. **Fixed 2026-09-24:** `hitPlayer` plays at (0, 0), z 40 (raw bytes checked by Claude: x and y stored from a zeroed r0); test in `gameplay.py`. **A zombie's hit and the girl's thank-you come from the centre.** `hitPlayer` plays at (0, 0), z 40, gain 1.0 (0x11f92..0x11fa2). Port: `monster_control.py:439-440` plays at the monster's position.
7. **Placement undone at the dev's request, 2026-09-24:** the click now goes off down the aimed lane with the shot (`_lane_pos`, z 0); clearing `shotFlag` at once stays. Before that, **fixed 2026-09-24:** `EMPTY_CLICK_POS` at z 40, `shotFlag` cleared at once; the grenade's at (0, 0), z 40. Claude traced every ammo check's branch in the raw bytes and found lanes 2 and 4 share lane 2's point (0x2f808), reproduced and recorded in DIVERGENCES. Test in `gameplay.py`. **The empty click (78).** The original clears `shotFlag` at once and plays down the aimed lane at z 40 (0x2fcee -> 0x2fdc2; grenade 0x2f508). Port: (0, 0), z 0, and it blocks the next shot for `ShotTime` (`stage_1_e.py:857-861`, 899-903).
8. **Fixed 2026-09-24:** `READ_COIN_COUNT_DELAY` 1.6, `READ_MINUTES_DELAY` 1.3, `READ_SECONDS_DELAY` 0.8, each double's high word read in the raw bytes by Claude; test in `menu.py`. The "2 s and 1 s" was a code comment in `app_delegate.readStop`, now corrected. **Coin row timing.** "After" -> minutes 1.3 s, not 2.0 (0x5e62..0x5e82); "minutes" -> seconds 0.8 s, not 1.0 (0x5dbc..0x5dd6); the count 1.6 s after "number of coins", not 1.5 (0x9bec..0x9c0c). Port: `app_delegate.py:274`, `:282`, `main_controller.py:225`. The DIVERGENCES line with 2 s and 1 s repeats the misreading.
9. **Fixed 2026-09-24:** `StartGame_` and `Store_`, the wrappers `tapCount` really sends (0x9420, 0x9438), click first; `StartGameAction_`'s own click, which the original never had, is gone. Test in `menu.py`, run with the full suite at the end of the batch. **Two missing clicks (10).** The Store row: `Store:` (0xb67c) clicks at 0xb6a8 before `StoreAction:`; port `main_controller.py:276-283` plays nothing. Start Game with no coin: `StartGame:` clicks at 0xad32 before 358; port clicks only when a coin is spent (`main_controller.py:295`).

**Not audible, for the developer tasks:** each walk tick sets `walkXFlag` and `timerLeft` clears it 0.6 s later (0x31f7a, 0x31f76/0x31f84); the port never sets it and clears after 0.0 s (`stage_1_e.py:486`). DIVERGENCES' Input section still says shaking free takes 10 presses, contradicting the 1 to 5 entry.

**Checked and fine:** the main loop (walk timer, breaths, alarm row 29, boss row 23, death, cell 8's girl, music 0.02), OpenAL setup (reference 40, max 800, cones, position order, stop-then-queue), the ambient layer (never sent, all zeros), combat (MovingShot guards and bands, 0.5 s hit delay, 330 and 79, melee, MonsterAttPlayer, shaking free, reloads), every MonsterControl method, the pause and result panel, the main menu, the shop and inventory sounds, the tutorial's prompts and timings, and the test range.

## Binary recheck, 2026-09-23 (after tunmi13productions' 17 commits, at `9ab032a`)

At the dev's asking ("recheck the binary for any more things we both mist"), two systematic sweeps instead of rereading by hand; the full suite passed 293 of 293 across 20 files first.
- **Methods the original sends that the port lacks.** Every method defined in a ported class and named in some listing's `SEL(...)` was checked against the port's defs. What is left is UIKit (nibs, views, loading bars, `tapSelect:`), the tutorial's per-beat methods (the port's table), and the server and StoreKit code. The rest, checked:
  - [V] `reloadShotgun` (0x3534c, a shell-at-a-time reload on `reloadShotGunTimer`) and `headerShot` are **never sent**: neither is in `__objc_selrefs` (checked with `tools/mb.py`'s `SELREF`). So `stopShotGunReload:` and `stopHeadShot`, sent only by them, are dead too. The port's single reload is faithful.
  - [V] `stopSoundArea:` is sent only by `mapPlotSound`, the ambient layer the shipped map never uses. `initUsingMark` and `MainController coinViewController` (run from `viewWillAppear:`) only hide or show images. `weaponDealloc` is sent only by the test range's end and restart, which rebuild the weapons anyway.
- **Sound numbers the original plays by literal that the port never names.** 95 literal numbers across the ported classes; three are not in the port:
  - 283 "animal zombies are coming" and 365 "please turn off" are only ever **stopped** (`tutorialSoundStop`, `MainController StopElseSpeak`), never played. Nothing missing.
  - [V] **340 "bitbee_1" is missing: the publisher's logo sound at launch.** `-[AppDelegate application:didFinishLaunchingWithOptions:]` plays it unconditionally at 0.2 (0x44f0/0x4502), puts up the logo view, fades it in over 2.5 s (0x459a `vmov.f64 d16, #2.5`) and on the animation's end calls `startIntro`, which fades it out over 1.0 s (0x49da) and calls `realStartIntro`, which only then builds `startIntroPage` (0x4af0). So the original is: logo sound at 0 s (the WAV is 4.00 s, stereo), the opening screen at 3.5 s, its welcome 2 s later at 5.5 s, after the logo sound has ended. The port opens straight on `startIntroPage`, so the welcome comes at 2 s and 340 is never heard. **Added the same day at the dev's word** ("I really want it"): `StartIntroPage.viewDidLoad` waits `LOGO_DELAY` (1.0 s: the dev asked for "500 to 1000ms", then settled on 1000), plays 340 (`play_logo`) and calls `realStartIntro` after `LOGO_SECONDS` (3.5). Enter skips the logo alone to the opening screen (`skip_logo`; `ScreenInput` passes Enter through while `logo` is set, since there is no row yet, which is why Enter first did nothing), Escape skips to the menu, both stopping 340, and Up and Down do nothing during it. Five tests in `tests/case/intro.py` (10 of 10); DIVERGENCES.md has "The logo plays on the opening screen, and can be skipped".

**The coin clock safety net, 2026-09-23.** The dev's save read "Number of coins, 0. The coin is charged after 0 minutes 0 seconds." Their `defaults.json` had `COIN` "0" and `FIREST` "1" but no `COIN_TIMER_START` or `COIN_TIMER`; they had edited it by hand to test coins. With no clock recorded, nothing ever starts one again: the original starts it only on spending a coin (0xbe01), and its time-away catch-up needs `COIN_TIMER_START` "1" (0x89a8). Not reachable in normal play, but a backup restored by the damaged-save recovery could leave it too. At the dev's word, `MainController._coinClockSafetyNet`, run from `viewDidLoad` after `_coinCatchUp`, starts the clock when `Coin < COIN_MAX` and `coinTimer` is None. Three tests in `tests/case/menu.py` (32 of 32), and the debug-mode start test now checks that Start leaves the clock as it was, rather than that no clock exists. In DIVERGENCES.md; the todo line waits in unfinished for the dev's confirmation.
...
- [V] **The double tap is off by one, as documented.** `tapCount` at 0x2ff26: `ldr r0,[r4,r0]` (selectMenu), `subs r0,#1`, `cmp r0,#7`, `bhi` to the exit at 0x2fff4, `tbb [pc,r0]` with `04 25 61 30 3b 4b 51 57`. Row 3 goes to the exit, row 4 to `ReadNumberOfHeadshot` (0x2ffa4), row 5 to `ReadObtainedGold`, and the top score row (10) falls past the bound; row 1 replays 229 in any state. **Fixed 2026-09-23 at the dev's decision** ("It should reread it."): `pause_activate` rereads the chosen row in both modes, row 1 through `pause_select(1)`, 3 `ReadNumberOfHeadshot`, 4 `ReadScore`, 10 `ReadTopScore`. Tests: `test_pause.test_choosing_a_result_row_rereads_that_row` and `test_choosing_the_first_row_says_its_own_state`.
...
- [V] **The power saw is an unfinished feature, not a bug.** `weaponInit` ends `adds r4,#1 / cmp r4,#8 / bne` (0x3512c), so slot 8 is never loaded. Its recordings 246 "a power saw button" and 254 "a power saw image" exist, but the shop (0x148f0) and the inventory (0x24a88) only ever stop 246; no row plays it. `DetailStoreController` has no weaponType 6 page and so no price; its only 6 is the Try table (0x1c1e4). `powersaw.plist` (50 entries) and sounds 74..77 are complete. **Settled 2026-09-23:** the dev said "Leave the powersaw alone", so it stays unused, as in the original; don't propose adding it.
...
Checked in the binary and found faithful, so not bugs:
- `-[MonsterControl StopPlayGame]` (0x10ca8..0x10d98) invalidates only `MainMonsterTimer`, `comingBreathTimer` and `MonsterMovingAngleTimer` and stops the note, the port's exact set; `headShotTimer` and any pending `headShot:` run on through a pause in the original too.
- `-[Stage_1_E gameReplayAction:]` touches no `gameMode`, `monster_num`, `LVCount` or `GirlMonsterNumber` (its IVOFF set is DieFlag, LVUP, monsterHPGain, gamePlayer, gameState, bStop, the run flags and the tutorial flags), so the [S] item in the 2026-09-22 rescan is closed as faithful: a restart keeps the area, tier and girl count it had.
...
## Status check, 2026-09-23 (every item below still marked open)

After the player todo list was emptied, every `[R]`, `[V]` and `[S]` line below without a fixed or closed mark was checked against the current code, and against the binary where it hinged on the original. **None is an open port bug.** The lines below keep their old wording as a record; this section settles them:
- **Fixed, the line just never said so:** `CheckTutorial` calls `MonsterAttPlayer` (`stage_tutorial.py` about 272); beat Eight sets `noAtt` before the grabber (`tutorial_sound_stop`, 0x8e1a8); `ReloadGesture` goes through the attack guards and does nothing for melee or the grenade; the replayed tutorial forces `isTutorial` to 0 (0x7cfd8); the tutorial's beats keep the original's order, and P ends it at beat Nine; "Now Loading" waits `LOADING_SECONDS`; `LVCount` resets on every attempt and cell 9 stores its tier; `MonsterComing_` takes its first step at once (0x1194c); melee plays att2 on a hit and att1 on a kill; a grab removes monsters after the loop (`done`); `MonsterKillCount_` counts the woman under `killMonster11count` (0x3a04c); the `MONSTER_SOUNDS` rows are full triples; the range clamp is 20 (0x11032); a heart is lost only while `isTutorial` is set (0x3b2e6, 0x3b79e); the boss and the level end are in `MainControl`; closing the window quits; the menu music no longer restarts (`MusicPlayer.play` leaves the same file playing); a save that is not an object no longer crashes (`defaults.py`, 2026-09-23).
- **Faithful, so not bugs:** `gameReplayAction_` leaves `gameMode`, `monster_num`, `LVCount` and `GirlMonsterNumber` alone, and so does the original: its restart (0x330ed..0x33902) writes only `DieFlag`, `LVUP`, `MotionSamplingTimer`, the walk and breath flags, `gamePlayer`, `gameState`, `isShake`, `monsterHPGain`, `noAtt`, `shotFlag` and the tutorial flags. The girl heals while `HP <= 3` (0x3b29c `cmp r0, #3 / bgt`), so four hearts are possible, as in the original.
- **Harmless:** `setListenerPos_` calls `alListener3f` where the original only stores the value (0xe9c8), but nothing calls it.
- **Deliberate:** Shift+Tab for the previous weapon is a keyboard addition (README's key table); the shake count now resets per grab by the dev's decision (1 to 5 presses).
- **Developer tasks, not player bugs** ([[project_dev_tasks]]): the run loop's three timing issues; the loading and path items (a bad `--game` falls back silently, notes past 122 are dropped without a log line, `ALCboolean` returns typed `c_int`, the device leaks if context creation fails, OpenAL is never closed on exit); the log file and `crash.txt`; the debug line missing F7; the stale prose; the tests writing the real save; the docs entries that are misreadings.
...
- [R] **No pause on focus loss, and no recovery when the audio device is lost.** The pause half was built 2026-09-22 and is not yet heard. The dev wants focus loss to work "just like pressing p". That is also what the original does: backgrounding posts InterruptON (0x4d90), and `interruptStop` calls `StopPlayAction:` (0x2c6ce). So SixthSense.py calls `ui/focus.interrupt_stop(obj)` on WINDOWFOCUSLOST in a stage or the tutorial: every time, now that continue clears `bStop`, and in the tutorial whatever P does. A first version froze the run loop and all audio instead; it was dropped at the dev's word, and its code was reverted. `tests/test_focus.py` covers it. The device half (`audioRestart` on InterruptOff) was built 2026-09-23 and confirmed by the dev the same day, by switching the default output in Sound settings and by unplugging headphones with the menu music playing: `AL.check_device` asks `ALC_CONNECTED` once a second and on focus regained, and reopens a lost device with `alcReopenDeviceSOFT` (OpenAL Soft 1.25.1 has both extensions); `tests/case/audio_device.py`. The dev's first try: unplugging worked, but the menu music stayed silent (a disconnect stops every source) and plugging back in stayed on the speakers (they never disconnect). So `check_device` now also moves when `ALC_SOFT_system_events` reports a default-output change (0x19D6; `alcEventIsSupportedSOFT` answers 0x19D9, supported, on WASAPI once the driver has started, and 0x19DA on the null driver), and starts again the looping sources that were playing at the last healthy check. The callback only sets a flag, and is let go at exit since OpenAL is never closed. Polling `ALC_DEFAULT_ALL_DEVICES_SPECIFIER` was rejected: after the first call it answers in 0.0 ms, so it looks cached.
- [R] **There is no crash path.** **Spoken half fixed 2026-09-23, confirmed by the dev:** `SixthSense.run` wraps `main`, and any exception, or a `SystemExit` carrying a message, is said in one line through `Speech` (a screen reader, or a Windows voice) and exits 1; tests in `tests/case/window.py`. The dev asked to keep it that simple ("just have it speak it"), so the log file and `crash.txt` stay a developer task. Was: nothing writes a log file, there is no `sys.excepthook`, and a missing data folder, DLL or audio device fails silently for a blind player. Log to `%APPDATA%\SixthSense`, write `crash.txt`, and speak the error.
...
- [R] **Continuing after a pause restarts the ambience and the music on the wrong players.** **Fixed 2026-09-23 as a divergence, the dev's choice (option 2 over the faithful notes):** `_pause_players` pauses whichever player is playing and `_resume_players` carries each on; `_resume_ambience` is gone. Byte-checked: the pause never calls `backgroundSoundStop` or `AMBSoundStop`. The music plays in every section, cell 10 to the next cell 9 (rows 669-601, 590-500, ... 23), not only past row 396; that row is only continue's note copy. Found in the 2026-09-21 rescan and traced in `dc_Stage_1_E.txt`, not yet byte-checked. It went into `todo list.txt` the same day.
...
- **Settled 2026-09-22:** the Windows port drops the coin store and the Game Center row. The dev also listed buying all weapons as part of the online shop, so restore purchases goes with it. Buying weapons one at a time stays. The todo list has these as removals. What to do with the ranking row, which plays 364 or is removed, is still the dev's call. **Settled 2026-09-23:** the coin store row and *Purchase all weapons* are removed too, at tsatria03's decision, instead of being sold for gold: "There are no human speech recordings for selling gold for coins." The shop's front menu is back, weapon shop and inventory, and the weapon list ends at the grenade. `test_store` covers it.
