...
**The first release was 26.09.23-1, on 2026-09-23**, made with `releaser.py` ([[project_release_tooling_plan]]). Before it, on 2026-09-22, the dev had removed a `26.09.20: Initial release.` entry because no release had happened then. The releaser files `unrelease:` under each new version; the compiler never does.
...
- **How big a release is (the dev, 2026-09-23).** A release aims for 50 to 100 changelog entries, fixes and enhancements alike, depending on how much the game still needs. It does not have to reach 100, and it can go out with fewer than 50 when there are not that many to make. **100 is a hard ceiling: never more than 100.** **And 5 is a hard floor (the dev, 2026-09-24): a version with fewer than 5 entries does not qualify for a release.** `releaser.py` enforces both (`MIN_ENTRIES`, `MAX_ENTRIES`): its check and its prepare step refuse more than 100, and none at all. **With 1 to 4, the dev asked the same day for a force override:** the releaser says how many there are and asks "Release anyway with N change(s)?" (`release_anyway`); Y goes on, anything else refuses as before. The check asks only when nothing else is in the way, and a full release asks once, in the check (`step_prepare(forced=True)` after it). The dev decides when a release is ready. The first release, **26.09.23-1**, went out on 2026-09-23 with 70 entries (the dev: "for this release, it's good with 70 changes"). That was 78 until the 8 debug mode lines moved out to [[project_dev_tasks]] the same day. Those 70 lines are now under the `26.09.23-1:` heading, which is released and must never be edited; `unrelease:` started again at 0 then. **The second release, 26.09.23-2**, went out on 2026-09-24 (04:56 UTC, still the 23rd by its date version) with 20 entries, the player readme first; the dev made it with the releaser, as the single-exe build, the first from `docks/changelog.txt`. `unrelease:` started again at 0 after it. **The third release, 26.09.24-1**, went out on 2026-09-24 (18:31 local, 01:31 UTC on the 25th) with 12 entries, the credits first, the first release under the 5-entry floor rule; the dev made it with the releaser (`a15d0b7`, a 118 MB zip). `unrelease:` started again at 0, and tunmi13productions' shot placement (`4f1f114`) made it 1 the same evening. **The fourth release, 26.09.24-2**, went out that same evening (03:12 UTC on the 25th) with that 1 entry, the first made with the force override (`d012556`). `unrelease:` started again at 0. Count the entries under `unrelease:` whenever one is added, and say so when the block nears 100, so the dev can release before it would pass the ceiling, and say so when it is still under 5. Never file the block under a version or run a build yourself ([[feedback_dont_run_or_build]]).
