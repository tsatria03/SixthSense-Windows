...
- **`compiler.py`**: the PyInstaller build script. Run it with no flags for a menu; it builds `dist\SixthSense-Windows` (around `SixthSense.exe`; the release zip extracts to that folder too), a folder build or with `--embed` one exe holding the sounds and data, and never zips or changes the repository ([[project_compiler_py]]).
