...
- **`tests/`**: plain scripts, each with its own runner. **They write the real save**, so read [[project_safe_test_run]] before running any. `tests/level_tester.py` is not a test: it starts the real game at any level, area and row, on its own save in `%APPDATA%\SixthSense\level_tester`, for checking by ear. `tests/tutorial_tester.py`, likewise not a test, does the same for the tutorial: any lesson, either ending, voice over on or off, on its own save in `%APPDATA%\SixthSense\tutorial_tester` ([[project_tutorial_tester_plan]]).
