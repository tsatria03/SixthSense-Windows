...
- **`game/`**: the original app bundle's data: the plists, the maps, the images and the iOS binary. Every sound the game uses lives in `game/sounds/used/`, in folders, under its original file name, a deliberate divergence. Since the dev's second sort (2026-09-24), `used/` holds only what the game plays (236 files), and `game/sounds/unused/` the 126 it never plays: the original's sounds for left-out features and unspawned zombies, extra copies, the non-original WAVs and one blooper clip. `SoundList.plist` entry 290 is renamed to match a renamed file, the only change to the original's plists. `paths.path_for_resource` looks in the top folder first, then by file name under `game/sounds/used/`, and last under `game/sounds/unused/`, so a name in `used/` always wins; builds carry both folders ([[project_sound_organization]]). Don't move, rename, convert or delete sound files unless the dev asks.
- **`analysis/`**:
...
- **`tests/`** ([[project_tests_layout]]): `tests/case/` holds the tests, plain scripts, each with its own runner. **They keep off the real save by themselves**: each imports `_scratch_save`, which sets `SIXTHSENSE_USER_DIR` to a throwaway folder, and `paths.py` fails if one does not. They still play audio, so read [[project_safe_test_run]] before running any. `tests/interact/` holds two tools played by ear. `tests/interact/level_chooser.py` is not a test: it starts the real game at any level, area and row, on its own save in `%APPDATA%\SixthSense\level_chooser`, for checking by ear. `tests/interact/tutorial_chooser.py`, likewise not a test, does the same for the tutorial: any lesson, either ending, voice over on or off, on its own save in `%APPDATA%\SixthSense\tutorial_chooser` ([[project_tutorial_tester_plan]]).
...
- **`releaser.py`**: sets the date version, files the changelog, runs the compiler, zips the build, commits, tags `V<version>` and uploads the zip to GitHub through `gh` ([[project_release_tooling_plan]]). Built and confirmed working by the dev on 2026-09-23.
