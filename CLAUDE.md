...
- **`docks/`**: the documents a player reads, which the build puts beside the executable: `readme.txt` (plain text for players, kept in step with what it describes, [[project_player_readme_plan]]), `changelog.txt` and `todo list.txt`.
